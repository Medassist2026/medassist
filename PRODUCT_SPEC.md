...
**Family / dependents UX — duplicate-minor dedup (codified by D-086, 2026-05-15).** When a guardian registers a minor dependent under their account, the system compares against existing minors under the same guardian using the tuple `(display name + date of birth + sex)`. If exactly one match exists, the existing dependent is reused — no duplicate row is created, the audit trail records a `GUARDIAN_LINK_REUSED` event, and the registration flow lands on the existing dependent's detail page. If the guardian genuinely has twins with the same name + DOB + sex (rare but legitimate Egyptian naming convention), the UI surfaces a 409-driven disambiguation modal showing both existing minors plus a "Create a new dependent anyway" affordance — selecting that resubmits with the `forceCreateNew: true` flag. The patient-app dependents list (`/patient/settings/family`) ALSO dedups defensively at read-time, so legacy duplicate rows that pre-date the K-1a write-time dedup collapse to a single visible entry; the dropped ids are preserved in the data layer for future cleanup tooling. This resolves Phase I.B Finding I-1 (v2 frontdesk onboard creating duplicate minor gps) and Finding I-18 (mother's `/patient/settings/family` showing TWO IDENTICAL "Aya Test" rows).

**OTP verification UX (codified by D-088, K-4, 2026-05-15).** OTP codes are **6 digits**, sent via SMS during (1) new-account phone verification at registration and (2) future password-reset recovery (Prompt 10 territory). Existing-user sign-in is password-only by spec (D-082); users encounter OTP rarely. The patient-app `/otp` page (and the clinic-app symmetric page) renders 6 input boxes with auto-advance + auto-submit on completion. The Arabic prompt reads "أدخل الرمز المكون من ٦ أرقام." Local `DEV_BYPASS_OTP=true` accepts any 6-digit input without going through SMS — used for staging + dev workflows; production deploys (per D-083) set `DEV_BYPASS_OTP=false` with a real SMS gateway (L-2 procurement).
