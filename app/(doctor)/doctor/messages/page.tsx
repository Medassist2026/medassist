          ...
          <div className="border-r border-gray-200">
            ...
            {conversations.length === 0 ? (
              <div className="p-8 text-center text-gray-500">
                ...
                <p className="text-sm mt-1">Messages from patients will appear here</p>
              </div>
            ) : (
              <div className="overflow-y-auto h-[calc(600px-57px)]">
                {conversations.map(conv => (
                  <div
                    key={conv.patient.id}
                    onClick={() => setSelectedPatient(conv.patient)}
                    className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 ${
                      selectedPatient?.id === conv.patient.id ? 'bg-primary-50' : ''
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-primary-700 font-semibold text-sm">
                          {conv.patient.full_name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <h3 className="font-medium text-gray-900 truncate">{conv.patient.full_name}</h3>
                    ...
                    >
                      ...
                      >
                        ...
                        <p className={`text-xs mt-1 ${
                          msg.sender_type === 'doctor' ? 'text-primary-200' : 'text-gray-500'
                        }`}>
