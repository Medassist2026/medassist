...
- **Admin Client Scope**: `createAdminClient(scope)` requires an explicit scope string for audit (D-008). Service-role admin endpoints additionally use `requireServiceRole(request)` — bearer-token timing-safe compare against `SUPABASE_SERVICE_ROLE_KEY`, returning 401 for any failure mode. The scope-tracking pattern has known drift — current `ALLOWED_ADMIN_SCOPES` covers 35 scopes while the codebase uses 135 distinct scope strings (210 total `createAdminClient(...)` invocations: 207 explicit-arg + 3 no-arg defaulting to `'api-route'`); validation is `console.warn` only. Reconciliation TRIAGED 2026-05-08 in `audits/admin-scope-reconciliation-2026-05-08.md` — Option D (sequenced hybrid: Phase 2 expand allow-list + mandatory eslint static-string discipline rule → Phase 3 TypeScript literal-union refactor → drop runtime check). Queued in `audits/PROGRAM_STATE.md` Phase F Task 16 (TRIAGED) and Task 20 (Phase 2 workstream).
