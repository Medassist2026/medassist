...
│
├── supabase/
│   └── migrations/                # 119 non-rollback migration files; highest base 116.
...
|-------|-----------|---------|-------|
| Framework | Next.js (App Router) | 15.5.18 | Server components + client islands. Both `apps/clinic` and `apps/patient` pin this. Root `package.json` also pins 15.5.18 (vestigial template residue — root has no `app/`/`pages/`/`next.config.js`; the four root scripts `dev/build/start/lint` reference `next` but are never invoked since CI uses workspace-scoped `build:clinic`/`build:patient`. Bumped from 14.2.35 to 15.5.18 in L-7 (2026-05-16) closing Dependabot Deferral A / D-093. Apps' `eslint-config-next` paired at 15.5.18. |
...
| Database | PostgreSQL (Supabase) | 17.6 | Staging on `medassist-egypt` (eu-central-1). RLS rewrite landed via mig 092-097 (incl. 094a) + forensic mig 106 + Phase D matrix prep mig 107 + Phase F Task 18 teardown fix mig 108 + B07 Phase B mig 109-112 (dependent accounts schema) + B07 Phase D mig 113-116 (authority helpers `is_authorized_actor_on` / `delegated_capability_includes` + OR-of-three RLS extensions on `global_patients` / `patient_clinic_records` / `patient_data_shares` / `audit_events`); 119 non-rollback migration files on disk (highest base 116). See §8.6. |
...
| SMS | Twilio (today's only vendor) — fronted by `SmsGateway` interface | — | OTP, appointment reminders, prescription delivery, share-expiring notifications, privacy-code SMS share consent. Per L-2-config / D-090, new SMS-emitting code uses the `SmsGateway` interface in `packages/shared/lib/sms/gateway.ts` (`ConsoleLogSmsGateway` when `DEV_BYPASS_OTP=true`; `TwilioSmsGateway` otherwise). Vendor adapters slot in next to `TwilioSmsGateway`. Legacy callers still use the lower-level `sendSMS` primitive in `twilio-client.ts` — migration is incremental. |
| PWA | next-pwa | 5.6 | Service worker, offline page, IDB cache + pending-writes queue (D-050). Clinic app only. |
| Error Tracking | Sentry | 10.38 | `@sentry/nextjs` integration. Wired L-4 / Bundle 6 (2026-05-16) via `apps/{clinic,patient}/instrumentation.ts` (server-side `initSentry()` on each runtime boot) + `SentryInit` client component mounted into each root layout (browser-side) + `Sentry.captureException` calls in both apps' `error.tsx` + `global-error.tsx`. PHI redaction via `beforeSend` (strips Authorization + Cookie headers) and Replay's `maskAllText` + `blockAllMedia`. No-op when `NEXT_PUBLIC_SENTRY_DSN` is unset (preview/staging without a DSN remain quiet). Source-map upload (`withSentryConfig` wrapper) deferred until Mo provisions `SENTRY_AUTH_TOKEN` — see `audits/phase-l-mo-walltime-tracker.md` §L-4. |
...
**Per-environment OTP / SMS behavior (D-090):**

| Environment | `DEV_BYPASS_OTP` | `SmsGateway` adapter | What happens |
|---|---|---|---|
| Production | unset / `false` | `TwilioSmsGateway` | Real SMS sent via Twilio (or future vendor adapter). Real OTP codes. |
| Preview / Staging | `true` | `ConsoleLogSmsGateway` | No SMS sent. OTP-bypass hint visible in UI. Logs the body that WOULD have been sent. |
| Local dev | `true` | `ConsoleLogSmsGateway` | Same as Preview. |

The Preview-env defaults are baked into each app's `vercel.json` `env` block; Production overrides supersede them via the Vercel dashboard's per-scope env-var settings. See `audits/phase-l-mo-walltime-tracker.md` for the Mo-side configuration checklist + L-2 SMS-gateway-procurement status.

**Two-project deployment table:**
...
**OTP digit count is 6 (K-4, 2026-05-15, D-088, Finding I-10).** The OTP generator (`packages/shared/lib/auth/otp.ts::generateOTPCode`) produces 6-digit codes via `crypto.randomInt(100000, 1000000)`. The two `/otp` pages (patient-app + clinic-app) render 6 input boxes; auto-advance to the next box on each digit; auto-submit when all 6 are filled. The verify-otp handler is length-agnostic (hash equality). OTP scope is narrow per D-082: registration verification + future password-reset recovery channel (Prompt 10 territory) — NOT existing-user login. See D-088 for the 4→6 bump rationale (healthcare-data security + industry alignment + D-082's rare-use framing).

**Patient-app entry layer alias (`/login`, K-4a, 2026-05-15, Finding I-8).** `apps/patient/app/(auth)/login/page.tsx` is a server-side redirect to `/auth?tab=login` (or `?tab=register` if the inbound URL carried that param). Pre-K-4 the directory was empty (I-8). Defensive against deep-link bookmarks, password manager autofills (which often default to `/login`), and browser history. Not a separate auth surface — `/auth` remains canonical per D-085.

**Phone storage convention (K-2d, 2026-05-15, D-087, Finding I-17).** Application schema (`public.*`) stores phones in canonical **`+E.164`** form (`+201XXXXXXXXX`). `auth.users.phone` stores **bare 12-digit** form (`201XXXXXXXXX`, no leading `+`) per immutable Supabase Auth convention — Supabase strips the `+` on insert and serves the bare form on read. Mig 089 (Build 04 D7) normalized 29 pre-existing `auth.users.phone` rows to match. Cross-schema bridge: `stripPlusForAuthUsers()` in `packages/shared/lib/utils/phone-normalize.ts` converts the public-schema `+E.164` form into the bare auth-side form for read-side comparisons; writes to `auth.users.phone` pass `+E.164` directly (Supabase strips). Two TS helpers exist for normalization with different output shapes — `phone-validation.ts::normalizePhone` returns the 12-digit form (legacy callers), `phone-normalize.ts::normalizeEgyptianPhone` returns the `+E.164` form (canonical, parity-tested against mig 071's `normalize_phone_e164` plpgsql function). See D-087.

**Minor dependent dedup (K-1, 2026-05-15, D-086).** Dependents have a load-bearing identity tuple: `(guardian_global_patient_id, display_name, date_of_birth, sex)`. When the same tuple is registered twice under the same guardian, we treat it as a duplicate by default and reuse the existing minor gp — both at **write time** in `createMinorGlobalPatient` (returns the existing id + emits `GUARDIAN_LINK_REUSED`, status 200) and at **read time** in `listDependentsByGuardian` (collapses by the tuple, oldest occurrence wins). Twins with the exact same name + DOB + sex are the legitimate escape hatch: the API accepts a `forceCreateNew: true` body flag that skips the dedup lookup and inserts a fresh row anyway. When the lookup finds 2+ existing matches the API returns 409 + `matchedIds[]` so the UI can render a "did you mean…?" disambiguation picker. The read-time dedup is defense-in-depth — even when K-1a's write-time dedup is in place, legacy data (pre-K-1a) may contain duplicates that need collapsing at the API boundary. See D-086 for the architectural rationale.

**Self-registration vs. clinic-presence creation (K-2c, 2026-05-15, D-084).** Patient self-registration via the patient app (`createPatientAccount` in `packages/shared/lib/data/users.ts`) writes **`auth.users` + `users` + `global_patients` only**. It does NOT create a `patients` row. The `global_patients` row is the canonical identity: `claimed=true`, `claimed_user_id` set to the new `auth.users.id`, `normalized_phone`, `display_name`. Clinic-presence rows (`patients` + PCR + DPR) are created by frontdesk on the patient's first clinic visit, NOT at signup — the patient hasn't visited any clinic yet, so `patients.clinic_id` and `patients.global_patient_id` (both NOT NULL since the TD-005 / D-041 multi-tenant rollout 2026-04-25) have no value to supply. Pre-K-2c, `createPatientAccount` tried to insert into `patients` and was architecturally broken for ~3 weeks; the failure went undetected because no real user has self-registered via the patient app (no production deployment per I-5 / D-083). **Read-path implication:** patient-app handlers query identity from `global_patients` via `claimed_user_id`; clinical-event data lives in the 35 tables that FK to `patients.id` (appointments, prescriptions, lab orders, vital signs, etc.) and returns empty for self-registered users until first clinic visit. UI must handle this empty state cleanly (verified Phase F + F.5 shipped this). **Frontdesk paths unaffected** — clinic-side patient creation (`createWalkInPatient`, dependent onboard via `establishMinorClinicPresence`) continues to create the full four-row shape (gp + patients + PCR + DPR) because the registering clinic IS the patient's first clinic-presence. See D-084 for the full ruling.
...
119 non-rollback migration files on disk; highest base is 116 (B07 Phase D RLS extension for OR-of-three authority on shares + audit_events, applied 2026-05-10). Several conceptual ranges are split into multiple files (e.g., 094a is a numbered amendment to 094); a handful of tracking rows reference SQL applied via dashboard with no file (backfilled by mig 100). For full file-vs-staging reconciliation see `audits/database-audit/staging-vs-mig-reconciliation.md`.
...
| 109-112 | B07 Phase B — Dependent Accounts schema (Patterns A + B) | Authored 2026-05-10 by the cowork session per the architectural review at commit `07fcbf8`. **109**: `global_patients` minor-gp additions — `guardian_global_patient_id UUID REFERENCES global_patients(id) ON DELETE SET NULL`, `is_minor BOOLEAN NOT NULL DEFAULT FALSE`, two CHECKs (`minor_requires_guardian`, `minor_no_self_claim`), partial index on `guardian_global_patient_id WHERE NOT NULL`. The "non-minor must have phone" CHECK was deliberately dropped (mig 076 already relaxed `normalized_phone` for the sentinel pattern; see Decision 1 in `audits/b07-phase-b-execution-2026-05-10.md`). Smoke probe POS + 3 NEG cases all PASS. **110**: `patient_delegations` table for Pattern B adult-to-adult delegation — 17 columns including `principal_global_patient_id`, `delegate_user_id`, `capabilities JSONB`, two-step grant/accept flow (`accepted_at` nullable), revoke + auto_renew columns, `metadata JSONB`. 2 CHECK constraints (`delegate_not_self`, `revoke_consistency`), partial unique index for active uniqueness (EXCLUDE/btree_gist fallback per Decision 2 — staging lacks btree_gist), 2 operational indexes, `updated_at` trigger. Capability tokens NOT validated at DB level (D-008 eslint-locked at app layer in Phase E). Smoke probe POS + 3 NEG + updated_at trigger check all PASS. **111**: backfill 3 legacy dependents — for each, set `is_minor = TRUE` and `guardian_global_patient_id`. Resolution paths: dep #3 (احمد محمد) via FK lookup through `patients.guardian_id`, dep #2 (نوح عمر) via phone lookup at `parent_phone`, dep #1 (فاطمة أحمد) via 3-step reconstruction (Decision 4) — release child's gp phone, INSERT new parent gp at `+201234567890` with placeholder `display_name = 'ولي أمر فاطمة أحمد'`, attach guardian. 4 audit events emitted with `actor_kind='migration'`. Legacy `patients` rows preserved (cleaned up at Prompt 6.5). The originally-conditional mig 112 (mig 081 compat-trigger amendment) was **skipped** — investigation found mig 081 operates only on clinical tables, not `patients`/`global_patients` (Decision 3); the slot was repurposed for the review-additions mig 112 below. **112** (NEW from Mo's review-additions batch, 2026-05-10): single CHECK constraint `patient_delegations_grantor_not_delegate_chk: granted_by_user_id <> delegate_user_id` (Decision 12). Closes a schema-design gap caught on review of the first Phase B surface — review §5.3's two-step grant→accept flow presumes grantor and delegate are different humans; mig 110 enforced delegate ≠ principal at the gp level but did not enforce grantor ≠ delegate at the user level. Mig 110's smoke probe was sympathetically updated in the same batch to gracefully skip (RAISE NOTICE + RETURN) when staging has fewer than 2 auth.users (the previous single-user fallback would now self-violate the new CHECK). Smoke probe POS + NEG both PASS. RLS matrix re-run at run_no=2.0 (post-all-four-mig): 177/177 PASS, all outcomes match run_no=1.6 baseline (no policy regression from any of the four schema additions). Empirical Lesson #19 codified from the mig 110 first-apply iteration (Decision 13): `NOW()` is transaction-start time, not wall-clock; smoke probes testing `BEFORE UPDATE` triggers on timestamp columns must seed historical timestamps. Applied 2026-05-10. |
| 113-116 | B07 Phase D — Authority helpers + RLS OR-of-three extensions | Authored 2026-05-09/10 by the cowork session per the architectural review at commit `07fcbf8`. **113**: Authority helper functions `public.is_authorized_actor_on(uuid, uuid DEFAULT auth.uid())` (INVOKER outer wrapper) + `public._is_authorized_actor_on_internal(uuid, uuid)` (DEFINER inner). The inner DEFINER bypasses RLS for the three branch joins (self-claim, guardian-link with `is_minor=TRUE` filter, active accepted-non-revoked-non-expired delegation); the outer INVOKER NULL-guards inputs and lets RLS policies invoke as `is_authorized_actor_on(id)` via the `auth.uid()` default. Capability-aware companion pair `public.delegated_capability_includes(uuid, uuid, text)` + internal: identical OR-of-three structure but branch 3 additionally requires `capabilities ? p_capability` (JSONB key existence). Both function pairs `STABLE PARALLEL SAFE` with `SET search_path = public, pg_temp`. No explicit REVOKE/GRANT (Phase D-E Decision 1 — would break the wrapper's INVOKER chain since EXECUTE is checked at call time regardless of DEFINER on the callee; matches mig 092 lines 330-338 precedent). Authority chain depth = 1 per ruling 7. 13 in-mig smoke-probe assertions PASS (8 `is_authorized_actor_on` cases + 5 `delegated_capability_includes` cases) using transient `00000113-...` UUID-prefixed fixtures. Helper family expansion: D-064's `3 DEFINER + 2 INVOKER` becomes `5 DEFINER + 4 INVOKER` total (mig 092 4 helpers + mig 094a `user_has_clinic_path_to_gp` + mig 113 4 helpers). **114**: ALTER POLICY in place on `global_patients`. SELECT extended with `OR public.is_authorized_actor_on(id, auth.uid())` — admits self / clinic-path / guardian-of-minor / active delegate. UPDATE narrowed extension: inline EXISTS for guardian-link only (Phase D-E Decision 7 — delegate excluded because profile-update has no MVP capability and `/api/patient/profile` does not call `requireCapability`). 4 SELECT-policy smoke probes PASS via `set_config('role','authenticated', TRUE)` + JWT claim spoof. **115**: ALTER POLICY in place on `patient_clinic_records`. SELECT extended with `OR public.is_authorized_actor_on(global_patient_id, auth.uid())` — guardians and delegates can read PCR rows for their authorized gp across clinics (Phase D-E Decision 6: RLS = authority, handler = capability). 3 PCR-SELECT smoke probes PASS. **116**: ALTER POLICY in place on `patient_data_shares` (SELECT broadens to OR-of-three; UPDATE/revoke broadens to guardian-link only per Decision 7) and on `audit_events_patient_self_select_v2` (extends with OR `is_authorized_actor_on(resolved_global_patient_id, auth.uid())`). INSERT policy on `patient_data_shares` left unchanged at `WITH CHECK false` per Phase D-E Decision 8 — prompt's INSERT-policy edit referenced `grantor_user_id` and `grantor_global_patient_id` columns that do not exist; share creation flows exclusively through `create_shares_for_grantors` RPC (mig 091) and capability gating (including post-MVP `consent_to_share`) belongs in the handler that calls the RPC. 4 smoke probes PASS (2 share + 2 audit). Full 177-scenario RLS matrix re-run deferred to Mac-side; spot-check at staging confirmed zero regression on existing personas (the new OR-clauses evaluate FALSE for personas without guardian/delegation relationships — additive permissive widening, not narrowing). Mig 113-116 applied to staging 2026-05-10 (timestamps `20260510024132` / `024504` / `024649` / `025012`). |
...
- **Admin Client Scope**: `createAdminClient(scope)` requires an explicit scope string for audit (D-008). Service-role admin endpoints additionally use `requireServiceRole(request)` — bearer-token timing-safe compare against `SUPABASE_SERVICE_ROLE_KEY`, returning 401 for any failure mode. **Phase 2 shipped 2026-05-09**: `ALLOWED_ADMIN_SCOPES` expanded from 35 → 136 entries (105 missing added, 4 truly-unused removed) AND a custom eslint rule `medassist-local/no-unregistered-admin-scope` (in `eslint-rules/`) now enforces static-string discipline at commit time — rejects any callsite scope not in the allow-list AND rejects any non-static-literal argument (template literals / variables / expressions). The runtime `console.warn` check in `admin.ts` stays as a defense-in-depth signal during the Phase 2 → Phase 3 transition. Phase 3 (Option C.1 TypeScript literal-union refactor) remains queued. The eslint rule's static-literal-only check carries forward into Phase 3 unchanged. See `audits/admin-scope-reconciliation-2026-05-08.md` (full inventory + Phase 2 shipped section) and `audits/PROGRAM_STATE.md` Phase F Task 20 (DONE 2026-05-09).
...
| TD-009 | **Server auth handlers still use inline E.164 regex instead of canonical phone validator.** Originally framed as drift risk; investigation during B07 Phase I uncovered Finding I-19 — the inline pattern `^\+2001[0125][0-9]{8}$` was **malformed** (extra leading zero, no real Egyptian +E.164 number matched). Production-mode (`DEV_BYPASS_OTP=false`) auth would have rejected every legitimate phone. Resolved by L-3 (2026-05-16) — `EG_PHONE_RE = /^\+20(10\|11\|12\|15)[0-9]{8}$/` and `E164_RE` now exported from `packages/shared/lib/utils/phone-validation.ts` and imported by both handlers; 28-test unit suite at `packages/shared/lib/utils/__tests__/phone-validation-eg.test.ts`. Storage-format canonicalization (170/288 users on local-11-digit form) remains a separate workstream, not gated by this fix. | `packages/shared/lib/api/handlers/auth/{login,register}/handler.ts`, `packages/shared/lib/utils/phone-validation.ts` | Original drift risk + latent production bug | **Resolved** 2026-05-16 — L-3 / Finding I-19. |
