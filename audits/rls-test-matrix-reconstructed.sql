...
-- Run target:  staging (mtmdotixlhwksyoordbl), records at run_no = 1.5
--
-- WHAT THIS FILE IS
-- -----------------
-- Executable PL/pgSQL test matrix that replays the 177 scenarios captured in
-- _rls_test_results WHERE run_no = 1, against the post-mig-106 staging RLS
...
--   4. RESET ROLE.
--   5. PERFORM public.rls_test_record(1.5, scenario, table, desc, expected,
...
-- USAGE
-- -----
--   1. Apply mig 107 first (already applied 2026-05-06; this matrix depends on
--      run_no being NUMERIC and rls_test_record having p_source_file).
...
--   3. The file is idempotent: it DELETEs run_no = 1.5 first, refreshes the
--      seed, then writes 177 fresh rows.
...
-- ============================================================================
-- Section 0 — Pre-flight: refresh seed + clear prior run #1.5
-- ============================================================================

DELETE FROM public._rls_test_results WHERE run_no = 1.5;
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'clinics'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'clinical_notes'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'conversations'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'doctor_availability'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'doctors'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'global_patients'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'messages'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'notifications'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'patient_clinic_records'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'patient_data_shares'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'patient_privacy_codes'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'payments'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
    ('S1', s_doctor_a,       'doctor_a clinic_a',                                 'SUCCESS', 1, 'READ',
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'privacy_code_attempts'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'privacy_code_sms_tokens'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'users'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  FOR rec IN SELECT * FROM (VALUES
    ('S1', s_doctor_a,       'doctor_a sees self',                                                'SUCCESS', 1, 'READ',
...
--
-- IMPORTANT — audit_events S6 expected_rows is computed DYNAMICALLY at the
-- start of this block. Reason: the rls-test-seed.sql teardown clause only
-- cleans audit_events with entity_type IN ('global_patients',
-- 'patient_data_share'), but the patient_data_shares + PCR INSERT triggers
-- create entity_type='patient_clinic_record' audit events on every seed
-- cycle. Each cycle leaves +4 rows behind across the three test gps. Run #1
-- captured 8/16/8 (cycle 8 of the historical seed-teardown loop); staging
-- today is at a higher cycle. The test's job is to assert that patient_y
-- sees EXACTLY the count postgres sees for their own gp, not a stale
-- historical snapshot — so S6's expected_rows is set to the live postgres
-- count at run time (v_y_baseline). S7a/b/c stay at 0 (FAIL) — leaks should
-- be 0 regardless of the cycle.
--
-- The comparison query against run #1 will report row-count divergence on
-- audit_events.S6 (8 vs whatever-now); outcome match (SUCCESS=SUCCESS) is
-- preserved. Documented in the run-end report so the divergence is
-- transparent and explainable as a seed-cycle artifact, not an RLS finding.
-- ============================================================================
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'audit_events'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  -- Compute live baseline for patient_y's audit events as postgres (RLS-bypassing role)
  SELECT count(*)::int INTO v_y_baseline
  FROM public.audit_events WHERE resolved_global_patient_id = s_patient_y_gp;

  FOR rec IN SELECT * FROM (VALUES
    ('S6',  s_patient_y_user,
       format('patient_y_user sees own audit rows via resolved_global_patient_id (%s expected from live baseline; seed-cycle drift vs run #1''s 8)', v_y_baseline),
       'SUCCESS', v_y_baseline, 'READ',
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'imaging_orders'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  SELECT pcr.id INTO s_pcr_x_at_a FROM public.patient_clinic_records pcr
   WHERE pcr.global_patient_id = s_patient_x_gp AND pcr.clinic_id = s_clinic_a;
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'lab_orders'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  SELECT pcr.id INTO s_pcr_x_at_a FROM public.patient_clinic_records pcr
   WHERE pcr.global_patient_id = s_patient_x_gp AND pcr.clinic_id = s_clinic_a;
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'lab_results'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  SELECT pcr.id INTO s_pcr_x_at_a FROM public.patient_clinic_records pcr
   WHERE pcr.global_patient_id = s_patient_x_gp AND pcr.clinic_id = s_clinic_a;
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'prescription_items'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  SELECT pcr.id INTO s_pcr_x_at_a FROM public.patient_clinic_records pcr
   WHERE pcr.global_patient_id = s_patient_x_gp AND pcr.clinic_id = s_clinic_a;
...
DECLARE
  ...
  rec RECORD; v_count INT; v_pass BOOLEAN; v_caught BOOLEAN; v_notes TEXT;
  v_table TEXT := 'vital_signs'; v_run NUMERIC := 1.5;
  v_src TEXT := 'audits/rls-test-matrix-reconstructed.sql';
BEGIN
  SELECT pcr.id INTO s_pcr_x_at_a FROM public.patient_clinic_records pcr
   WHERE pcr.global_patient_id = s_patient_x_gp AND pcr.clinic_id = s_clinic_a;
...
-- FROM public._rls_test_results WHERE run_no = 1.5
...
--        r15.actual_outcome AS run15_outcome, r15.actual_rows AS run15_rows
-- FROM public._rls_test_results r1
-- LEFT JOIN public._rls_test_results r15
--   ON r15.run_no = 1.5
...
-- Expected outcomes:
--   run #1.5 aggregate: pass = 177, fail = 0, total = 177
--   comparison query: 0 rows
