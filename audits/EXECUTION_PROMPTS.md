...
**Correction 2026-05-07 (later, post-investigation):** the empirical-justification paragraph above misattributes the operational fix. Subsequent investigation (CI run 25535878629, six consecutive failed runs 115–120 from 2026-05-07 03:38 UTC through 22:00+ UTC) surfaced that runs 118 (`aa2b991`), 119 (`134e272`), and 120 (`f766a05`) all failed AFTER `9774252` had been merged. If `9774252`'s webpack-alias additions had been the operational fix, run 118 (the next build after that merge) would have turned the matrix green; it didn't. The actual root cause — and what was failing under run 25475031898 too — was a route-handler type contract issue: `packages/shared/lib/api/handlers/patient/sharing/handler.ts` line 27 declared `export async function GET(request?: Request)` (optional first arg) at commit `61f8752` (Build 05, 2026-05-04). Next.js's route-handler contract rejects `Request | undefined` at `next build` time. The fix was a one-character source change (`?` removal) in commit `80ee270`; CI run 25539467112 against that commit is green. What `9774252` actually contributed was (a) the build-log capture mechanism in `ci.yml` that surfaced the error string to artifacts — necessary for diagnosis six runs later — and (b) preventive webpack-alias declarations that are defensible discipline (the third declaration site eliminates a known failure class for `@patient/*` cross-segment imports the moment such an import exists), but were not the active bug fix. The standing rule below remains correct as a forward-looking discipline rule: Next 14.2.x's webpack resolver genuinely does not always honor tsconfig path aliases for cross-segment imports inside an app, and registering aliases at the third level prevents a real failure class. The empirical paragraph above is preserved as a record of what was believed when commit `134e272` shipped; this correction records what the runs-115–120 data actually proved. The route-handler-contract failure mode is codified as Empirical Lesson #17 (`tsc --noEmit` does not enforce Next.js route-handler type contracts).
...
### Lesson 17 (CI fix-attribution reconciliation, 2026-05-07)

**`tsc --noEmit` does not enforce Next.js route-handler type contracts; only `next build` does.**

**Context:** Build 05 patient sharing lifecycle (commit `61f8752`, 2026-05-04) shipped `packages/shared/lib/api/handlers/patient/sharing/handler.ts` with `export async function GET(request?: Request)` — the optional `?` on the first parameter makes its type `Request | undefined`. The pre-push `tsc --noEmit` gate (D-045) passed. The CI Lint-and-Type-Check job passed. The CI Build Patient App job failed at the route-validation phase with `Type "Request | undefined" is not a valid type for the function's first argument` — a Next.js-specific contract that requires the first arg to be exactly `NextRequest` or `Request`, not a union with `undefined`. Six consecutive CI runs (115 through 120) failed silently from 2026-05-07 03:38 UTC through 2026-05-07 22:00+ UTC, every one at the same Build Patient App step with the same opaque `Process completed with exit code 1`. The build-log-capture mechanism added in commit `9774252` is what made diagnosis possible — without artifact uploads, the failing job's output was opaque. The fix landed in commit `80ee270` (one-character `?` removal); CI run 25539467112 against that commit is green.

**Lesson:** `tsc --noEmit` enforces general TypeScript correctness but does not validate Next.js's route-handler type contracts. Next's contract checker runs only inside `next build` — it inspects the type of every `app/**/route.ts` export (and any handler re-exported into a route file) and rejects parameter signatures that don't match `(request: NextRequest | Request, ...)`. Optional first args (`request?`), wider unions including `undefined` or `null`, default values, and rest-parameter destructuring all fail this check at build time but pass `tsc --noEmit`. The pre-push gate that relies on `tsc --noEmit` alone is therefore necessary but not sufficient for route handlers — it cannot guarantee CI green.

**Standing rule:**

1. **Route handlers and the handlers they re-export must accept their first argument non-optionally as `request: Request` or `request: NextRequest`.** This applies to files matching `app/api/**/route.ts` and to files in `packages/shared/lib/api/handlers/**` whose exported HTTP-method handlers (`GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`) are re-exported into a route. No `?` on the first parameter. No default value. No `undefined` in the type union.
2. **Reviewers reject any `?` on the first parameter of a route-handler-shaped export.** The fact that `tsc --noEmit` accepts it is not a defense; the gate that catches it runs in CI.
3. **If the body's logic conditionally uses `request`, narrow inside the function** (e.g. `if (request) { … }` or a runtime guard on `request.url`) rather than expressing optionality at the parameter level. The `b05/sharing/handler.ts` GET handler used `request ? new URL(request.url) : null` after the `?` removal — that pattern is harmless and was left in place to keep the diff minimal.

**Operational follow-up flagged for Phase F discussion** (out of scope for this lesson): consider adding `next build` to the pre-push gate, or running it as a fast-fail CI step before the full multi-app build. Trade-off: pre-push latency (a full `next build` is much heavier than `tsc --noEmit`) vs. CI feedback time (currently the route-handler contract failure shows up only deep inside the multi-minute Build Patient App job). One option is a narrow `next build --no-lint --no-mangling`-style invocation scoped to route validation only; another is leaving the discipline at the reviewer level and accepting the CI feedback latency. Decision deferred.

**Empirical proof:** Commits `61f8752` (2026-05-04, introduced the bug) and `80ee270` (2026-05-07, fixed the bug) bracket six consecutive CI Build Patient App failures (runs 115–120, all visible at https://github.com/Medassist2026/medassist/actions). Pre-push `tsc --noEmit` passed clean across all of them. CI Lint-and-Type-Check passed clean across all of them. Only Build Patient App failed. CI run 25539467112 against `80ee270` is green. The build-log-capture discipline shipped in `9774252` was a necessary-but-insufficient input to the diagnosis: necessary because without the artifact upload the error string was invisible; insufficient because the mechanism alone produced no fix — that took a separate commit. Pairs with **Lesson #14** (path-alias three-level discipline — initially miscredited as the operational fix; corrected in the 2026-05-07 (later) annotation), **Lesson #13** (lockstep doc maintenance — drove this reconciliation), and the operational-follow-up question above.

### Lesson 18 (Dependabot closure-count framing error, 2026-05-09)

**Filter by event timestamp, not by net delta, when measuring closures.**

**Context:** Phase F Task 16 batch (commit `39c2f43`, 2026-05-08) bumped root `next` 14.1.0 → 14.2.35 to close the critical `CVE-2025-29927` Dependabot alert plus 6 other root-only `next` advisories. The D2 surface predicted "7 alerts expected to close. Net post-push: 55 → 48." When the post-push state showed 50 open alerts (not 48), the next session's prompt initially framed it as "predicted 7 closures, actual 5; 2 stragglers are GHSA-5j59 + GHSA-mwv6." That framing was wrong on two levels: (a) the "5" was a net delta, not a closure count — Dependabot rechecked exactly 7 hours after push and closed exactly 7 alerts (`fixed_at = 2026-05-09T03:15:29Z`, all 7 GHSAs match D1's predicted-7 list verbatim); (b) GHSA-5j59 + GHSA-mwv6 were never in the predicted-7 list — they're 4-manifest GHSAs whose root-manifest entry had already been quietly closed by a 2026-05-07 Dependabot tracking-model migration; conflating the predicted-7 (single-manifest closures) with the broader §6.1 "closes at root level" list (multi-manifest GHSAs whose root entry closes but lockfile + app entries stay open) produced the wrong-stragglers narrative. The actual arithmetic: 55 open at yesterday's snapshot − 7 closures (D1's prediction held 100%) + 2 post-snapshot arrivals (`fast-uri v39h` + `@babel/plugin-transform-modules-systemjs fv7c`, both created `2026-05-09T03:15:30Z`) = 50 open today. The misread blocked clean Phase 2 start: today's prompt was authored on a wrong-prediction-failure premise, requiring a correction section in the triage doc + a reframed Task 1 before Phase 2 work could begin.

**Lesson:** When measuring closures from an action, filter by event timestamp (`fixed_at >= action_timestamp` or equivalent state-transition timestamp), not by net counts (`len(open_before) - len(open_after)`). Net delta is corrupted by parallel arrivals from independent sources — upstream advisory publication, workspace dependency rescans, manifest tracking reorganizations, auto-dismissals, deduplications. The rule applies to Dependabot alerts, CI runs, monitoring incidents, support tickets — anything where items can both close AND be created during the observation window. The closure count IS knowable cleanly; net delta IS NOT.

**Standing rule:**

1. **Verification queries filter by event timestamp.** For Dependabot specifically: `jq '[.[] | select(.fixed_at >= "<push-time>") | .security_advisory.ghsa_id]'`. For CI: `gh run list --json conclusion,headSha,createdAt | jq '[.[] | select(.createdAt >= "<event-time>")]'`. For tickets: filter by `closed_at >= action_time` not `count(open) before/after`.
2. **Author the verification query in the original prediction surface.** When the original surface predicts "X items will close from Y action," embed the post-action verification query in that surface so the post-action check is one-paste-distance away. Don't trust prose-only predictions — the predicted closure count must be backed by a query that retrieves it.
3. **Reviewers reject any "X items closed" claim sourced from a net-delta computation.** Sources of truth are the per-item state-transition timestamps; aggregations of them are derived data. If the surface presents a closure count without the underlying query, ask for it.

**Operational corollary:** the same discipline pattern (lock today's empirical precondition forward via a gate) appears in Phase F Task 20's eslint-rule design — "0 dynamic scopes today" became the static-literal-only rule, in the same way "filter by timestamp" became the verification-query embedding. Both are instances of "don't let an empirical snapshot drift silently."

**Empirical proof:** Phase F Task 16 batch (commit `39c2f43`) closed exactly 7 Dependabot alerts as D1 predicted (all at `fixed_at = 2026-05-09T03:15:29Z`, manifest=`package-lock.json`, GHSAs match D1's predicted-7 list verbatim). Net `length()` delta of open-state alerts showed -5 because 2 unrelated advisories (`fast-uri v39h` + `@babel/plugin-transform-modules-systemjs fv7c`, both created `2026-05-09T03:15:30Z`) were created in the same recheck window. The "5 closures, 2 stragglers" framing in my D2 follow-up surface was wrong; the correct read was "7 closures + 2 arrivals" (or "+3 arrivals" if one counts the `fast-uri q3j6` alert created during yesterday's pre-work session as a third parallel arrival from the broader push-to-recheck window). See `audits/dependabot-triage-2026-05-08.md` §0.5 + §0.6 for the empirical correction. Pairs with **Lesson #16** (verify against ground truth — Lesson #18 is the operational *how* when ground truth is a count) and **REVIEW_CRITERIA §2.1** (the verification table includes a "Closure counts" row pointing at this lesson).

### Lesson 19 (B07 Phase B mig 110 smoke probe iteration, 2026-05-10)

**`NOW()` is transaction-start time, not wall-clock; smoke probes testing trigger advancement on timestamp columns must seed historical timestamps.**

**Context:** B07 Phase B mig 110 (`patient_delegations` table) authored a smoke probe to verify the `BEFORE UPDATE` trigger `patient_delegations_touch_updated_at` advances `updated_at` on UPDATE. First apply attempt failed: `RAISE EXCEPTION 'mig 110 smoke probe POS-2: updated_at trigger did not advance the timestamp'`. The trigger function is `NEW.updated_at := NOW()` — structurally identical to existing patterns (`touch_global_patients_updated_at`, `patient_clinic_records_touch_updated_at`). The bug was in the smoke probe assertion: it INSERTed (default `updated_at := NOW()`), captured `v_initial_updated`, called `pg_sleep(0.05)`, UPDATEd, captured `v_post_update_updated`, and asserted `v_post_update_updated > v_initial_updated`. Within a single transaction, `NOW()` returns the **transaction-start timestamp**, identical at every call site; `pg_sleep` does not advance it. So `v_initial_updated == v_post_update_updated` even though the trigger fired correctly. (`clock_timestamp()` is the wall-clock alternative — but the trigger uses `NOW()` to match the project-wide pattern, and changing the trigger to `clock_timestamp()` would be a deviation, not a fix.)

**Fix (committed in mig 110):** seed `updated_at = '2000-01-01T00:00:00Z'::timestamptz` explicitly at INSERT (overriding the column default), then UPDATE, then assert `v_post_update_updated <> v_initial_updated` (and additionally that `v_post_update_updated >= NOW() - INTERVAL '1 minute'` to confirm the new value is "close to NOW()", not some other arbitrary timestamp). The seeded historical value is unambiguously different from the `NOW()` the trigger sets, so the probe passes when the trigger fires and fails when it doesn't.

**Standing rule:** **Any smoke probe testing a `BEFORE UPDATE` trigger on a timestamp column must seed a historical timestamp at INSERT, then UPDATE, then assert the trigger advanced the value past the seed.** Do not rely on `pg_sleep` between INSERT and UPDATE — sleeps don't advance `NOW()` within a single transaction. Do not use `>` comparisons against `NOW()` for the "post-UPDATE" check unless paired with a "close to NOW()" sanity bound; an `=` comparison against the seeded historical value is the cleanest assertion shape.

**Pairs with:**
- **Lesson #2** (smoke-probe assertion discipline — "the probe failed" must reflect a real schema bug, not a probe-authoring bug). Lesson #19 is a specific failure mode of probe authoring that Lesson #2's general rule allows.
- The existing `touch_*_updated_at` trigger pattern across `global_patients`, `patient_clinic_records`, etc. — those triggers are correct as written; do NOT change them to `clock_timestamp()` to make naïve smoke probes pass. Fix the probe.
