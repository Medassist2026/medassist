...
// ============================================================================
// ATTACHMENT DECODING (shared with doctor side)
// ============================================================================

function decodeMessage(content: string): { text: string; attachment: { url: string; name: string; type: 'image' | 'file' } | null } {
  try {
    ...
    if (parsed._att) {
      ...
      return {
        text: caption || '',
        attachment: { url, name, type: mime?.startsWith('image/') ? 'image' : 'file' }
      }
    }
  } catch { /* plain text */ }
...
function toAr(specialty: string) {
  return SPECIALTY_AR[specialty] ?? SPECIALTY_AR[specialty?.toLowerCase()] ?? specialty
}
...
// ============================================================================
// AVATAR — doctor initial with teal color
// ============================================================================
...
function DoctorAvatar({ name }: { name: string }) {
  ...
  return (
    <div className="w-12 h-12 rounded-full bg-[#DCFCE7] flex items-center justify-center flex-shrink-0">
      <span className="font-cairo text-[14px] font-bold text-[#16A34A]">{initials}</span>
    </div>
  )
}
  ...
  if (conversations.length === 0) {
    return (
      <div className="text-center py-20 px-6">
        ...
        <p className="font-cairo text-[16px] font-semibold text-[#030712] mb-2">لا توجد محادثات</p>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between gap-2 mt-0.5">
                <p className={`font-cairo text-[12px] truncate ${conv.unread_count > 0 ? 'text-[#4B5563]' : 'text-[#9CA3AF]'}`}>
                  <span className="text-[#9CA3AF] text-[11px]">{toAr(conv.doctor.specialty)} · </span>
                  {preview || '📎 مرفق'}
                </p>
                {conv.unread_count > 0 && (
...
function ChatView({
  ...
  const handleFileSelect = (file: File) => {
    if (file.size > MAX_ATTACHMENT_SIZE) { setUploadError('حجم الملف كبير — الحد الأقصى 5 ميجا'); return }
  ...
  const handleSend = async () => {
    if (pendingFile) {
      try {
        ...
        const { error } = await sb.storage.from('attachments').upload(path, pendingFile.file, { contentType: pendingFile.file.type })
        if (error) throw error
        const { data: urlData } = sb.storage.from('attachments').getPublicUrl(path)
        const content = encodeAttachment(urlData.publicUrl, pendingFile.file.name, pendingFile.file.type, newMessage.trim())
        if (pendingFile.preview) URL.revokeObjectURL(pendingFile.preview)
        setPendingFile(null)
        onSend({ content })
      } catch { setUploadError('فشل رفع الملف — حاول مرة أخرى') }
    } else {
      onSend()
    }
  }
  ...
  return (
    <div className="flex flex-col h-screen">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-[#E5E7EB] safe-area-top">
        <button onClick={onBack} className="w-[36px] h-[36px] rounded-full border-[0.8px] border-[#E5E7EB] flex items-center justify-center flex-shrink-0">
          <ChevronRight className="w-[20px] h-[20px] text-[#030712]" />
        </button>
        <DoctorAvatar name={doctor.full_name} />
        <div className="flex-1 min-w-0">
          <h3 className="font-cairo text-[15px] font-semibold text-[#030712] truncate">{doctor.full_name}</h3>
          <p className="font-cairo text-[12px] text-[#9CA3AF]">{toAr(doctor.specialty)}</p>
        </div>
        {isPolling && (
      ...
      {/* Info banner */}
      <div className="mx-3 mt-2 px-3 py-2 bg-[#FEF9C3] border border-[#FDE047] rounded-[10px]">
        <p className="font-cairo text-[11px] text-[#713F12]">
          📋 هذه المحادثة خاصة بينك وبين طبيبك — يمكنك إرسال صور التحاليل أو الأسئلة الطبية
        </p>
      </div>
                    ...
                    attachment.type === 'image' ? (
                      <div>
                        <a href={attachment.url} target="_blank" rel="noopener noreferrer">
                          ...
                          <img src={attachment.url} alt={attachment.name} className="max-w-[200px] rounded-[10px] mb-1 object-cover" />
                        </a>
                        {text && <p className="font-cairo text-[13px]">{text}</p>}
                      </div>
                    ) : (
                      <a href={attachment.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-black/10 rounded-[10px] px-3 py-2">
      ...
      {pendingFile && (
        <div className="px-4 py-2 bg-[#F0FDF4] border-t border-[#BBF7D0] flex items-center gap-3">
          {pendingFile.preview ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={pendingFile.preview} alt="preview" className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
          ) : (
          ...
          <div className="flex-1 min-w-0">
            <p className="font-cairo text-[13px] font-medium truncate">{pendingFile.file.name}</p>
            <p className="font-cairo text-[11px] text-[#6B7280]">{(pendingFile.file.size / 1024 / 1024).toFixed(1)} MB</p>
          </div>
          <button onClick={() => { if (pendingFile.preview) URL.revokeObjectURL(pendingFile.preview); setPendingFile(null) }} className="p-1">
      ...
      {uploadError && (
        <div className="px-4 py-1.5 bg-red-50 border-t border-red-200">
...
export default function PatientMessagesPage() {
  ...
  const loadMessages = useCallback(async (doctorId: string) => {
    try {
      ...
      const data = await res.json()
      if (data.messages) { setMessages(data.messages); setIsPolling(true) }
    } catch { /* silent */ }
  }, [])

  useEffect(() => {
    if (!selectedDoctor) { setIsPolling(false); return }
        ...
        {selectedDoctor ? (
          <>
            <ChatView
              ...
              sending={sending}
              isPolling={isPolling}
              onBack={() => { setSelectedDoctor(null); setMessages([]); setSendError('') }}
              onMessageChange={(m) => { setNewMessage(m); setSendError('') }}
              onSend={sendMessage}
            />
            {sendError && (
          ...
          <>
            {/* Header */}
            <div className="flex items-center gap-3 px-4 pt-safe pt-4 pb-3 border-b border-[#E5E7EB]">
              <div className="w-[36px] h-[36px] rounded-full bg-[#DCFCE7] flex items-center justify-center flex-shrink-0">
                <MessageCircle className="w-[18px] h-[18px] text-[#16A34A]" />
              </div>
              <div>
                <h1 className="font-cairo text-[18px] font-semibold text-[#030712]">رسائلي</h1>
                <p className="font-cairo text-[12px] text-[#9CA3AF]">محادثاتك مع أطبائك</p>
              </div>
            </div>
            <ConversationList
