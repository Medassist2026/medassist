...
import { validateEgyptianPhone, normalizePhone } from '@/lib/utils/phone-validation'
import { isAccountDormant, getDormancyPeriod } from '@/lib/utils/identity'
...
export async function verifyPatientCode(
  ...
  // Verify code matches
  if (patient.unique_id !== code.toUpperCase().trim()) {
...
export async function onboardPatient(
  ...
  if (data.patientCode) {
    ...
    if (verification.valid) {
      ...
      const { data: registeredPatient } = await adminSupabase
        .from('patients')
        .select('*')
      ...
      if (registeredPatient) {
        ...
        const { data: newRelationship } = await adminSupabase
          .from('doctor_patient_relationships')
          .insert({
            doctor_id: doctorId,
        ...
        return {
          success: true,
          patient: registeredPatient as Patient,
          relationship: newRelationship as DoctorPatientRelationship,
          isExisting: true,
          isGhostMode: false,
          message: 'Patient verified and added to My Patients'
        }
      }
    }
  }
  ...
  if (anyExistingPatient) {
    // Patient exists - create relationship (walk_in type since no code)
    const accessType = data.patientCode ? 'verified' : 'walk_in'
    ...
    const { data: newRelationship } = await adminSupabase
      ...
      .insert({
        ...
        verified_at: accessType === 'verified' ? new Date().toISOString() : null
      })
      .select()
      .single()
  ...
  // ============================================
  return await createWalkInPatient(doctorId, {
...
export async function createWalkInPatient(
  ...
  // ============================================
  // STEP 5: Create walk_in relationship
  // ============================================
...
export async function getMyPatients(
  ...
  const { data: relationships, error: relationshipsError, count } = await adminSupabase
    .from('doctor_patient_relationships')
    .select('*', { count: 'exact' })
    .eq('doctor_id', doctorId)
  ...
  const { data: patientRows, error: patientsError } = await adminSupabase
    .from('patients')
    .select('*')
    .in('id', patientIds)
...
export async function searchMyPatients(
  ...
  const { data: relationships, error: relationshipsError } = await adminSupabase
    .from('doctor_patient_relationships')
    .select('patient_id')
    .eq('doctor_id', doctorId)
...
export async function getPatientVisits(
  ...
  const relationAccessType = (relationship as any).access_type
  const relationStatus = (relationship as any).status
  const allowHistorySharing = relationAccessType === 'verified' || relationStatus === 'active'
...
export async function upgradeRelationship(
  ...
  // Verify code
  if (patient.unique_id !== patientCode.toUpperCase().trim()) {
...
export async function canMessagePatient(
  ...
  // Current schema: active relationship allows messaging.
