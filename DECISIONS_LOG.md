...
> **Amendment 2026-05-10 (B07 Phase E — API-side capability enforcer + 6 new admin scopes)**: Phase E ships the API-side enforcer for the `AllowedCapability` literal union: `requireCapability(globalPatientId, capability, userId)` in `packages/shared/lib/auth/authority.ts`. The TS literal union (compile-time) and the `no-unregistered-delegation-capability` eslint rule (lint-time) covered Phase C. The Phase E API handler adds a third rejection layer at the request-validation boundary: `POST /api/patient/delegations` and `PATCH /api/patient/delegations/[id]/capabilities` explicitly reject the string `'consent_to_share'` with a structured `CAPABILITY_POST_MVP` error code (Mo ruling 4 — post-MVP), separate from the generic "not in allowed set" path. This three-layer enforcement (TS / lint / runtime) is the canonical pattern for all future tokenized scopes. **Admin-scope allow-list grows by 6 in the same Phase E batch (148 → 154)**: 6 new B07 Phase E call sites — `authority-resolve` (the OR-of-three resolver in `requireAuthorityOver`), `authority-capability` (the RPC client in `requireCapability`), `dependents-register-resolve-gp` (caller's claimed-gp lookup in the register handler), `delegations-create-resolve-principal` (caller's principal-gp lookup in the create handler), `delegations-revoke-discriminate` (read-before-write to determine `revokedBy` for the response), and `cron-expire-stale-delegations` (the new daily cron). Same kebab-case naming convention; grouped under a new `// ── B07 Phase E API surface (authority helpers + endpoint handlers) ──` divider in `packages/shared/lib/supabase/admin.ts`. lint:scopes is the gate: a missing scope blocks pre-push.
...
### Amendment 2026-05-10 (B07 Phase E — API surface consumes the helper family)

The Phase E API handlers consume the mig 113 helper family from the application layer, completing the helper invocation chain. Two surfaces:

1. **`requireAuthorityOver(globalPatientId, userId)` in `packages/shared/lib/auth/authority.ts`** mirrors the OR-of-three predicate of `is_authorized_actor_on()` but at the TS layer. Three sequential SELECTs (self-claim, guardian-link, active delegation) with short-circuit on first match. Returns the matched `AuthorityBasis` (`self` / `guardian_of_minor` / `delegated_by_principal`) plus the `delegationId` when basis is delegated. The TS replication is intentional: the SQL helper returns BOOLEAN only, so the OR-of-three is opaque to callers; handlers want the basis for branch-specific authorization (e.g., dependent-detail endpoint requires basis = `'guardian_of_minor'`) and audit-row attribution (`metadata.acting_as`, `metadata.authority_grant_id`). The TS replication does NOT replace the SQL helper — the helper continues to gate the patient-side leg of every modified RLS policy (mig 114-116).

2. **`requireCapability(globalPatientId, capability, userId)`** short-circuits when basis is `self` or `guardian_of_minor` (implicit full capability per architectural review §3.5) and RPCs to `delegated_capability_includes()` only for the delegation branch. The RPC pattern (vs. re-implementing the JSONB-contains predicate in TS) keeps the capability-membership semantics in lock-step with whatever the SQL helper encodes — a future change to capability semantics lands in mig 113's body and the TS layer inherits it without modification.

Both helpers throw 403-status `ApiAuthError` subclasses (`AuthorityError`, `CapabilityError`) so the existing `toApiErrorResponse()` mapper handles them uniformly. Decision 4 in `audits/b07-phase-e-execution-2026-05-10.md` walks the alternatives.

The 5-DEFINER + 4-INVOKER helper family remains unchanged. No new SQL helpers, no new mig. Phase E is API/handler-only.
...
### Amendment 2026-05-10 (B07 Phase E — API surface ships)

The API handlers and authority helpers consuming D-068's substrate are now live. Three things change in how the directional-consent + delegation lifecycle is exercised:

1. **Authority helpers `requireAuthorityOver` and `requireCapability` ship in `packages/shared/lib/auth/authority.ts`.** They mirror the Phase D mig 113 SQL helpers at the application layer with two additions: (a) they report which OR-of-three branch matched (`self` / `guardian_of_minor` / `delegated_by_principal`), so handlers can branch on the basis (e.g., `GET /api/patient/dependents/[id]` requires `guardian_of_minor` specifically); (b) for the delegation branch they return the `delegationId` so handlers can attach it to audit rows as `metadata.authority_grant_id`. The TS short-circuits self/guardian (implicit full capability per architectural review §3.5) and only RPCs to `delegated_capability_includes()` for the delegation branch. Both errors extend `ApiAuthError(403)` so `toApiErrorResponse()` handles them uniformly without per-handler `instanceof` plumbing.

2. **9 patient-app endpoints + 1 cron + 1 modified clinic endpoint.** Dependents (Pattern A): `POST/GET /api/patient/dependents`, `GET /api/patient/dependents/[id]`. Delegations (Pattern B): `POST /api/patient/delegations` (defaults `capabilities=[]` per Mo ruling 18, `expiresAt = +1y` per Mo ruling 20), `PATCH /[id]/accept`, `PATCH /[id]/revoke`, `PATCH /[id]/capabilities`, `GET /granted`, `GET /received`. Cron: `apps/clinic/app/api/cron/expire-stale-delegations` (audit-only per Phase C Decision 8 — does NOT mutate `revoked_at`; the `expires_at` predicate in `is_authorized_actor_on` already filters expired grants). `/api/patients/onboard` gains an `isDependent: true` early-branch that bypasses the legacy `patients` table per Phase B Decision 3 and creates a minor gp via `createMinorGlobalPatient`.

3. **Bridge pattern for staff-driven minor onboarding (Phase E Decision 8).** `createMinorGlobalPatient` enforces `createdByUserId === guardian.claimed_user_id`. The clinic-side `/api/patients/onboard` is doctor/frontdesk-driven, so the staff member's `auth.uid()` does NOT match the parent's claim. The Phase C bridge resolves this: the handler looks up the parent gp by `parentPhone`, asserts the parent gp is claimed (and not itself a minor), and passes `guardian.claimed_user_id` as `createdByUserId`. The audit row attributes to the parent — semantically "the parent authored this minor's creation, even though staff typed it in" — matching MVP convention for staff-mediated patient consent. Unclaimed parent gps surface a structured 400 (`PARENT_UNCLAIMED`); inline parent-gp creation is rejected (per the Phase E onboard prompt — guardian must register first).

Validation belt-and-suspenders preserved: TS literal union (`AllowedCapability`) at compile time, eslint rule `no-unregistered-delegation-capability` at lint time, `validateCapabilities()` runtime check at the data-layer boundary, AND a third API-handler-layer rejection of `'consent_to_share'` (Mo ruling 4 — post-MVP) so the user-facing error is informative rather than a generic "not in allowed set" message.

Phase F (patient-app UI) is the next workstream and consumes this surface.
...
- Vuln count: pre-Bundle-7 = 59 (24 high, 28 moderate, 7 low); **post-Bundle-7 after Dependabot async re-scan completed = 3 (1 high, 2 moderate, 0 low)** — a 56-vuln net reduction. (Earlier this entry said `post-Bundle-7 = 60`; that was GitHub's push-time cached count from the PREVIOUS lockfile scan. Dependabot re-scans the new lockfile asynchronously after a push; the re-scan completed sometime between the Bundle 7 push at `feae943` and the Phase L closure push at `dcf8f18` (this commit), at which point GitHub's display dropped to 3. Empirical lesson: never read the vuln count off the push-time `remote:` line — it's the OLD scan. Wait for Dependabot's re-scan to complete (typically a few minutes to a few hours post-push). The "significant drop" the Bundle 7 plan predicted DID materialize; the cached display at push-time hid the win. Future migration expectations should account for this reporting lag.)
