...
export function AppointmentsList({ 
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`text-xs px-2 py-0.5 rounded-full ${styles[status as keyof typeof styles] || styles.scheduled}`}>
    ...
    <div className="space-y-4">
      ...
      {showStatusGuide && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <HelpIcon 
              content="Click an appointment to start a clinical session with that patient"
              position="right"
            />
          </div>
          <button
      ...
      {showLegend && (
        <StatusLegend 
          ...
          title="Appointment Status Guide"
        />
      )}
      ...
      <div className="flex gap-2 border-b border-gray-200 pb-2">
        {[
          ...
          { key: 'upcoming', label: 'Upcoming', count: appointments.filter(a => a.status === 'scheduled' || a.status === 'confirmed').length },
