...
export function hashNationalId(nationalId: string): string {
  const salt = process.env.NATIONAL_ID_SALT
  if (!salt || salt.trim().length < 16) {
    throw new Error('NATIONAL_ID_SALT is required and must be at least 16 characters')
  }
  const cleaned = nationalId.replace(/[\s-]/g, '')
...
export function verifyNationalId(input: string, storedHash: string): boolean {
  ...
  return timingSafeEqualString(inputHash, storedHash)
}
